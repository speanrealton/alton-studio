...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('🔄 Update status request - userId:', userId, 'isActive:', isActive);

    if (!userId || isActive === undefined) {
      return NextResponse.json(
        { error: 'User ID and status are required', type: 'validation' },
        { status: 400 }
      );
    }
    ...
    console.log('✅ Status updated successfully to:', isActive);
